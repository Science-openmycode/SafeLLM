...
from aloepri.attacks.protocol import load_target_tau_for_scoring, score_mapping_predictions
from aloepri.evidence import run_provenance


def main() -> None:
    parser = argparse.ArgumentParser(description="Isolated scorer for mapping-attack predictions")
    parser.add_argument("--predictions", type=Path, required=True)
    parser.add_argument("--target-key-dir", type=Path, required=True)
    parser.add_argument("--out", type=Path, required=True)
    args = parser.parse_args()
    predictions = json.loads(args.predictions.read_text(encoding="utf-8"))
    metrics = score_mapping_predictions(
        predictions, load_target_tau_for_scoring(args.target_key_dir)
    )
    payload = {
