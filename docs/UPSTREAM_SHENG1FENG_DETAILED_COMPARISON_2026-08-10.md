...
4. 本地代码已形成可复现 Git 基线，但尚未配置 GitHub 远程仓库，也没有最终 `release/` 交付包。
...
当前分支为 `codex/aloepri-baseline`。基线只跟踪源码、配置、测试和文档；以下内容被明确排除：

- 模型 checkpoint、在线密钥和离线主密钥；
- `data/`、`artifacts/`、`outputs/` 和临时上游仓库；
- 甲方 PDF、PPTX、DOCX 原件；
- 本地虚拟环境、缓存和日志。

提交前门禁结果：Ruff 通过，Mypy 检查 47 个源码文件通过，Pytest 为 120 项通过、1 项真实模型测试按环境变量跳过。真实 0.5B 模型测试已有独立历史证据，但本次 Git 基线提交没有重新加载模型，以免把模型文件或运行产物纳入版本库。
