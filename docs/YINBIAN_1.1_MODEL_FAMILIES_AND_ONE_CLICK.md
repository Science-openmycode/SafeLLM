...
yinbian plan create --model qwen3-4b --mode direct-deploy --device auto --server <server-id>
...
|---|---|---|---|
| Qwen2/Qwen2.5 | GQA、RoPE、RMSNorm、Dense SwiGLU | Qwen 私有运行时 | 全系列按 checkpoint 再校验 |
...
| GLM Dense | GQA、partial RoPE、fused Gate/Up | 规范化后进入 Qwen 私有运行时 | 规范化不执行仓库远程代码 |
...
real Qwen2.5-0.5B private API round-trip: 1 passed (CPU)
ruff: pass
mypy: pass (128 source files)
```

跳过项是需要显式环境变量才能运行的真实 Qwen2.5-0.5B 模型集成测试。现有真实
Qwen 工件和历史验收数据没有被新单元测试替代。
